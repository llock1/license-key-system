...
const copyKey = (keyId) => {
    ...
    const key = keyArray.find(item => item.ID === keyId);
    if (key) {
      ...
      toast.error("Key not found", 2000);
    }
}
  ...
  <div class="relative overflow-x-auto">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      ...
      <tr v-for="key in keys" :key="key.ID" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
        ...
        <td class="px-6 py-4">{{ key.Hwid || 'N/A' }}</td>
        <td class="px-6 py-4 flex flex-row items-center justify-center gap-2">
          <button class="text-red-500 cursor-pointer" @click="deleteKey(key.ID)">
          ...
          <button class="text-white cursor-pointer" @click="copyKey(key.ID)">
